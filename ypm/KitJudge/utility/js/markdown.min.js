!function(b) {
	...
	c.prototype.split_blocks = function(a) {
						...
						for (var s = a.split(/(?=\n)/), t = "", u = 0; u < s.length; u++) {
							...
							if (e = w.match(x), void 0 !== e[1]) {
								...
								if (y > m.length) n = f(e), o.push(n), o = n[1] = ["listitem"];
									...
									var z = !1;
									for (r = 0; r < m.length; r++) if (m[r].indent == e[1]) {
		...
		"[": function J(a) {
			...
			if (h) {
				var i = h[1];
				if (e += h[0].length, i && "<" == i[0] && ">" == i[i.length - 1] && (i = i.substring(1, i.length - 1)), !h[3]) for (var j = 1, k = 0; k < i.length; k++) switch (i[k]) {
				case "(":
					j++;
					break;
				case ")":
		...
		for (var b = i(a), c = {}, d = 0; d < b.length; ++d) if (/^#/.test(b[d])) c.id = b[d].substring(1);
	...
	b.renderJsonML = function(a, b) {
		...
		else for (a.shift(), !a.length || "object" != typeof a[0] || a[0] instanceof Array || a.shift(); a.length;) c.push(o(a.shift()));
		return c.join("\n\n")
	}
}(function() {
