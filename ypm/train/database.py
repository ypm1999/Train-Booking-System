...
def try_login(id, password):
    ...
    while(True):
        ...
        if not line:
            break
        temp = line.split(' ')
        if temp[0] == id and temp[2] == password:
            return temp[0]
    return None
...
def query_profile(id):
    ...
    while True:
        ...
        print temp
        if temp[0] == id:
            return {'id':temp[0], 'name':temp[1], 'password':temp[2], 'email':temp[3], 'phone':temp[4], 'admin':(temp[5] == '1'), 'sucessed':True}
...
def modify_profile(id, name, password, email, phone):
    for line in fileinput.input('user.tmp', inplace = True):
        ...
        if num == id:
            print "%s %s %s %s %s %d" % (id, name, password, email, phone, pri)
        else:
            print(line)
    else:
        return False
